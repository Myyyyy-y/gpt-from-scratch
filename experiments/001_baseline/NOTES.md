...
## Goal
验证全链路（BPE → memmap 数据 → 手写模型 → 手写训练栈）在真实数据上端到端可用，建立后续所有实验的对照锚点。

## Setup
- 模型：16M（6 层 / d_model 384 / 6 头 / d_ff 1344，RMSNorm + SwiGLU + RoPE，权重绑定）
- 数据：TinyStories 全量 train（4.67 亿 token，自训 BPE vocab 8192）
- 训练：28500 步（1 epoch），bs 64×256，lr 3e-4（warmup 200 + cosine 到 3e-5），bf16
...
## Limitations
- lr 3e-4 偏保守（后续 LR sweep 证实 1e-3 更优）
- 曲线末端仍在下降，1 epoch 未充分收敛
- 16M 容量对 4.67 亿 token 的数据量偏小（29 token/参数 > Chinchilla 20）

## Next steps
- 提高 lr + 放大模型 → 即 002/003/004/007 的 29M LR sweep
