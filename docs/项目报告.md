...
结论：最优 lr 在 1e-3 附近的平缓盆地；发散组完整留档（`assets/lr_sweep.png`）。
与两个参考项目（1e-3 / 1.25e-3）互证。

### 位置编码消融（rope / learned / none，29M + lr 1e-3）

结果待训练完成后填入（进行中）。早期信号（同 step 7750 对比）：
rope 1.6396 < learned 1.6739 << none 1.7939（step 5250）。
