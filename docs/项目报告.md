...
- **踩坑记录**（工程与训练两侧均有留档，详见 `docs/训练技术验证报告.md` 附录）：
  工程侧：Embedding 初始化与权重绑定冲突（初始 loss 273）、CPU autocast dtype 校验、
  HF split 名 valid/validation、空 corpus 复用、transformers 版本冲突、data3m 被 pkill 误杀续训等；
  训练侧：QK-Norm 组 min_lr 混淆导致假结论（干净归因后修正）、Muon 缺 weight decay 发散、
  LayerNorm 中间快照误判、KV cache 小模型加速比被 kernel 启动开销稀释等

## 第五部分：局限与下一步

- 多 seed 显著性：冠军组 seed 1/2/3 best 均值 **1.3771 ± 0.0108**（单次 1.3668, 1.3761, 1.3883，seed=0 冠军 1.3832）
